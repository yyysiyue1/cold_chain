...
DEFAULT_MODEL_FILENAME = "pork_Methylquinoline_model.pkl"

class Pork_Methylquinoline_Predictor:
    def __init__(self, model_pkl: Optional[str] = None) -> None:
        """
        如果传相对路径，则相对于本文件所在目录解析；
        若不传，则默认加载同目录下 DEFAULT_MODEL_FILENAME。
        """
        base_dir = Path(__file__).resolve().parent  # .../models/pork_Methylquinoline
    ...
    def predict_step(self, C_prev: float, T: float, t_prev, t) -> float:
        ...
        # 调试输出（可注释）
        print(f"Methylquinoline---dt_h={dt_h:.6f}h, T={T}, C_prev={C_prev}, t={t}, t_prev={t_prev}")
