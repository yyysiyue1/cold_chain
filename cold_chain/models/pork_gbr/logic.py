...
from .pork_gbr_predictor import Pork_gbr_Predictor
...
def _compute_gbr_value(row: Dict[str, Any], food_info: Dict[str, Any], engine, predictor: Pork_gbr_Predictor) -> float:
...
def execute(
    ...
    # 1) 懒加载/复用模型
    predictor: Pork_gbr_Predictor = predictor_cache.get("pork_gbr")
    if predictor is None:
        predictor = Pork_gbr_Predictor()
    ...
    low = RiskClassifier.pork_gbr_low
    mid = RiskClassifier.pork_gbr_mid
    high = RiskClassifier.pork_gbr_high
    if value is None:
        risk_level = "无"
